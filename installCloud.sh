...
pacman -Sy postfix dovecot nginx php php-fpm php-gd php-mcrypt php-intl php-apcu wget
...
if [[ ! -z "$owncloudDataPath" ]]; then
	echo "open_basedir = /srv/http/:/home/:/tmp/:/usr/share/pear/:/usr/share/webapps/:$owncloudDataPath" >> /etc/php/php.ini
fi
...
case "$choice" in 
  n|N ) printText "Type the connection information for the existing ownCloud database"
		...
		read -s -p "Type password for user $owncloudUser:" owncloudPassword
